...
fn emit(p: &[f64], k: f64, train_mse: f64, val_mse: f64, mode: Mode) {
    let today = "2026-06-06";
    // The mode tag makes every candidate file self-describing: a pin-material
    // run carries K = 1.520 like a default run and would otherwise be
    // header-indistinguishable (review finding, ea546a3).
    let mode_tag = match mode {
        Mode::Default => "default",
        Mode::FitK => "fit-k",
        Mode::PinMaterial => "pin-material",
    };
    println!("//! GENERATED by `cargo run --release --bin tune`. Do not edit.");
    println!(
        "//! (retuned {today}; mode = {mode_tag}; K = {k:.3}, train MSE = {train_mse:.6}, val MSE = {val_mse:.6})"
    );
...
fn main() {
    ...
    emit(&p, k, final_train, final_val, mode);
}
